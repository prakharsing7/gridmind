...
def test_grid_constraints_defaults():
...
def test_grid_constraints_zero_power_raises():
    with pytest.raises(ValueError):
        GridConstraints(max_site_power_w=0.0)


def test_price_period_end_before_start_raises():
    with pytest.raises(ValueError, match="end must be after start"):
        PricePeriod(start=BASE, end=BASE - timedelta(hours=1), price=0.25)


def test_price_signal_valid():
    ps = PriceSignal(
        periods=[
...
def test_price_signal_empty_raises():
    with pytest.raises(ValueError):
        PriceSignal(periods=[])
